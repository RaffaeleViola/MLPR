...
def FusionEvaluation():
    ...
    DCF.append(act_DCF(scores_fused, labels, 0.5, 1, 1))
    # Evaluation set
    fuser.fit(np.vstack([rbfsvm_scores, gmm_scores, lr_scores]),
              labels)  # train on validation scores noKFOLD whole dataset
    ...
    # Best models on evaluation set Fusion
    rbfsvm_scores, labels = KFold_CV(DTR, LTR, K, SVM.RBFSVM,
                                     wpoint=wpoints, pca_m=0, pre_process=None, p_T=0.5, C=10, k=1, gamma=0.001)
    rbfsvm = SVM.RBFSVM(p_T=0.5, C=10, k=1, gamma=0.001)
    rbfsvm.fit(DTR, LTR)

    lr_scores, _ = KFold_CV(DTR, LTR, K, LogisticRegression.QuadraticLogisticRegression,
                            wpoint=wpoints, pca_m=0, pre_process=None, lmd=0.02, prior=0.7)
    lr = LogisticRegression.QuadraticLogisticRegression(lmd=0.02, prior=0.7)
    lr.fit(DTR, LTR)
    ...
    table = PrettyTable()
    table.set_style(MARKDOWN)
    table.field_names = ['Model', 'evaluation_minDCF', 'evaluation_actDCF']
    ...
    DCF.append(min_DCF(scores_fused_eval, LTE, 0.5, 1, 1))
    DCF.append(act_DCF(scores_fused_eval, LTE, 0.5, 1, 1))
    table.add_row([f'RBFSVM + GMMTied + QLR', *DCF])
