...
fig = plt.figure()
plt.plot(lambdas, minDCF_validation[0], color="red", label="p_T = 0.5 [Dev Set]")
plt.plot(lambdas, minDCF_evaluation[0], color="red", linestyle='dashed', label="p_T = 0.5 [Eval Set]")
plt.plot(lambdas, minDCF_validation[1], color="blue", label="p_T = 0.2 [Dev Set]")
plt.plot(lambdas, minDCF_evaluation[1], color="blue", linestyle='dashed', label="p_T = 0.2 [Eval Set]")
plt.plot(lambdas, minDCF_validation[2], color="green", label="p_T = 0.8 [Dev Set]")
plt.plot(lambdas, minDCF_evaluation[2], color="green", linestyle='dashed', label="p_T = 0.8 [Eval Set]")
plt.xlabel("Lambda")
plt.ylabel("minDCF")
plt.suptitle("LogReg Eval Valid Comparison")
plt.xscale('log')
plt.xlim(lambdas[0], lambdas[-1])
plt.legend()
plt.savefig(f'{absolute_path}/../Images/LRPlot/LogRegEvalValid.png')
plt.close(fig)
...
table = PrettyTable()
table.set_style(MARKDOWN)
table.field_names = ['Model', 'PCA(m)', 'pi = 0.5', 'pi = 0.2', "pi = 0.8", 'zs_pi = 0.5', 'zs_pi = 0.2', "zs_pi = 0.8"]
...
for m in [0, 12, 11, 9, 8]:
    ...
    clf = LogisticRegression.QuadraticLogisticRegression(lmd=100, prior=0.7)
    clf.fit(DTR_, LTR)
